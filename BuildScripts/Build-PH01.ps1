...
if(-not $ArchiveDirectory){$ArchiveDirectory=Join-Path $heldRoot 'Builds/PH01'}
...
if($CreateAssets){
 & "$EngineRoot/Engine/Binaries/Win64/UnrealEditor-Cmd.exe" $heldProject -run=pythonscript "-script=$PSScriptRoot/create_traversal_assets.py" -unattended -nop4 -NullRHI
